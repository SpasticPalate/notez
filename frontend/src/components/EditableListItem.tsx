    ...
    if (onDrop) {
      try {
        ...
        if (rawData.length > 10000) {
          if (import.meta.env.DEV) console.warn('Drop data exceeds size limit');
          return;
        }
  ...
  return (
    ...
    >
      {isEditing ? (
        ...
        // View mode
        <>
          <button onClick={onSelect} aria-current={ariaCurrent} className={`flex items-center space-x-3 flex-1 ${indent ? '' : ''}`}>
