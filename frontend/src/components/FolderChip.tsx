...
import { ChevronDown, Folder, Plus, Check, Loader2 } from 'lucide-react';
...
export function FolderChip({ folders, selectedFolderId, onChange, onRefresh, disabled = false }: FolderChipProps) {
  ...
  const handleCreateFolder = async () => {
    ...
    try {
      const response = await foldersApi.create({ name: trimmedName, icon: newFolderIcon });
        ...
        >
          ...
          {isCreating ? (
            <div className="px-3 py-2 space-y-2">
              {/* Row 1: Icon picker + folder name input */}
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  maxLength={255}
                  disabled={isSubmitting}
                  className="flex-1 min-w-0 px-2 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 disabled:opacity-50"
                />
              </div>
              {/* Row 2: Cancel and Create buttons */}
